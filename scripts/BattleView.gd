...
func _draw_build_preview(lane_y: float) -> void:
	...
	var color := Color(0.30, 0.94, 0.60, 0.24) if valid else Color(1.0, 0.30, 0.40, 0.24)
	draw_circle(Vector2(mouse_position.x, lane_y - 28.0), 42.0, color)
	draw_arc(Vector2(mouse_position.x, lane_y - 28.0), 42.0, 0.0, TAU, 40, color.lightened(0.45), 2.0)
	draw_line(Vector2(mouse_position.x, lane_y - 70.0), Vector2(mouse_position.x, lane_y + 5.0), color.lightened(0.5), 1.0)
