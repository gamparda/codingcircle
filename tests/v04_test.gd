...
func _init() -> void:
	...
	if SaveData != null:
		...
		expect_eq(defaults.settings.window_size, "1920x1080", "new profiles default to 1080p")
		expect_eq(defaults.settings.fps_limit, 60, "new profiles default to 60 FPS")
		expect_eq(defaults.settings.graphics_quality, "high", "new profiles default to high quality")
		var old_visuals: Dictionary = SaveData.sanitize({"settings": {"window_size": "1280x720", "fps_limit": 144, "effect_intensity": 0.65}}).settings
		expect_eq(old_visuals.graphics_quality, "custom", "old manual graphics settings are not overwritten")
		expect_eq(old_visuals.window_size, "1280x720", "old resolution is preserved")
		expect_eq(old_visuals.fps_limit, 144, "old FPS preference is preserved")
		var low: Dictionary = SaveData.graphics_profile("low", true, Vector2i(720, 1280))
		expect_true(low.fps_limit == 30 and not low.battle_effects and low.window_size == "1280x720", "low profile reduces effects and frame limit")
		var medium: Dictionary = SaveData.graphics_profile("medium", false, Vector2i(1920, 1080))
		expect_true(medium.fps_limit == 60 and medium.battle_effects and not medium.screen_shake and medium.window_size == "1600x900", "medium profile uses balanced effects")
		var high: Dictionary = SaveData.graphics_profile("high", false, Vector2i(1920, 1080))
		expect_true(high.fps_limit == 60 and high.battle_effects and high.screen_shake and high.window_size == "1920x1080", "high profile keeps 1080p 60 FPS")
		expect_eq(SaveData.graphics_profile("auto", true, Vector2i(720, 1280)), low, "auto chooses low for small mobile screens")
		expect_eq(SaveData.graphics_profile("auto", true, Vector2i(2400, 1080)), medium, "auto chooses medium for large mobile screens")
		expect_eq(SaveData.graphics_profile("auto", true, Vector2i(1080, 2400)), medium, "auto accounts for portrait screen orientation")
		expect_eq(SaveData.graphics_profile("auto", false, Vector2i(1920, 1080)), high, "auto chooses high on desktop")
		...
		expect_eq(safe_settings.window_size, "1920x1080", "unknown window sizes recover to default")
