...
func _init() -> void:
	...
	if UpdateManager != null:
		...
		if has_policy:
			...
			expect_true(updater.accept_manifest(manifest_bytes), "validated unsigned manifest enters update flow")
			expect_true(updater.state == "waiting_safe", "newer manifest waits for safe point")
			updater.state = "checking"
			expect_true(not updater.accept_manifest("{}".to_utf8_buffer()), "malformed manifest is rejected by update flow")
			expect_true(updater.manifest.is_empty() and updater.state == "failed", "malformed manifest fails closed")
